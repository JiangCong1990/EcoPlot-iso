...
inp <- read.csv("Demnitz_inpmod_Cp.csv",na.strings="", sep = ",")     		    # read input into dataframe from .csv file with , as decimal separator
...
parRange <- data.frame(min=c(-0.6, 0.1, 40, 40,  1,  1,  1,  50, 250, 1.0, 1.0, 1.0, 0.1, 0.5, 1,  3,  10,  0.25, 0.25, 0),
                       max=c(-0.1, 2.0, 60, 60, 20, 20, 20, 100, 450, 5.0, 5.0, 5.0, 0.9, 1.0, 20, 40, 100, 0.90, 0.75, 2))     # specify parameter ranges from min to max for alpha and beta parameters of the gamma df
...
observed<- read.csv("Demnitz_inpmod_Cp.csv", sep = ",")
