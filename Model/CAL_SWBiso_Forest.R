...
SWMc <- function (         # Model parameter definition
  ...
  beta = 0.1,        # extinction factor for root density function
  inp	
)             				
...
{  ##### BEGIN OF FUNCTION BODY #####
  ...
  # Extract variables from the input data frame
  P <- inp$P
  NR <- inp$NR
  AT <- inp$AT
  RH <- inp$RH
  ...
  SW_10_D <- inp$SW_10_D
  SW_30_D <- inp$SW_30_D
  SW_100_D <- inp$SW_100_D
  TempS_10 <- inp$TempS_10
  TempS_30 <- inp$TempS_30
  TempS_100 <- inp$TempS_100

 # Define the log file path (you can adjust this path as needed)
  log_file <- "Ecoplot.log"

  # Use a unique identifier for the log entry
  process_id <- Sys.getpid()  # Unique process ID
  timestamp <- Sys.time()     # Timestamp for logging
  
  # Create the log message
  log_message <- paste("Q_DE has been called by process", process_id, 
                       "at", timestamp)
  
  # Append the message to the log file
   cat(log_message, file=log_file, append=TRUE, sep="\n")
...
#  }
  ...
  for (i in 1:nrun) {					# the loop iterates through the length of the input time series
    ...
    da_H[i] <- (-44.9 - k*epse_H[i])/(1+k*epse_H[i]*10^-3) 
  ...
  #  API[i] <- 0.995*API[i] + P[i]        # daily decay constant of 0.95 by Hill et al. (2014)
    ...
    if(PN[i] > 4.2){                        # In larger net precipitation events (>P80) preferential flow allowed
...
}              # Upper store reduced accordingly
 
#####      Tp[i]<- Tp[i] - Tr_Upper[i]                # Potential transpiration reduced accordingly, no need anymore
      ...
      Tr_Upper[i] <- min(Tr_Upper[i], STO[i]) # Ensure that transpiration does not exceed available storage
 
      STO[i]<- STO[i] - Tr_Upper[i]
#####      Tp[i]<- 0  # use the Tp_Upper_residual
         }   
...
####        Tp[i]<- 0
    ...
    if (i==1){
      ...
      fInt_CD[i] <- ((Int_D[i] * Int_CD[i-1]) - (Ei[i] * Idl_H[i])) / (Int_D[i] - Ei[i])
    ...
    if (i==1){
      ...
      # compute vapor isotopic composition (Craig and Gordon 1965 formula, with notation by Gibson 2016)
      fupCSTO_D[i] <- ((upSTO_D[i] * upCSTO_D[i-1]) - (Es[i] * Sdl_H[i])) / (upSTO_D[i] - Es[i]) 
    ...
    # calculate atmospheric water vapour mixture
    if ( Ei[i] + Es[i] + Tr[i] == 0)
    { WV_D[i] <- 0}
    else { 
      WV_D[i] <- ((IdE_H[i] * Ei[i]) + (SdE_H[i] * Es[i]) + (Tr_Upper[i] * fupCSTO_D[i]) + (Tr_Lower[i]*gwCSTO_D[i]) + (Tr_Deep[i]*lowCSTO_D[i])) / (Ei[i] + Es[i] + Tr[i])  # simulated water vapour isotope composition from interception and soil evaporation plus transpired water
    }
  }  
  # objective functions for calibration
  Qtemp1  <- na.omit(data.frame(SW_10[366:nrun], STO[366:nrun]))								        # create time frame without missing discharge time steps
  Q_Rsqu <- cor(Qtemp1$SW_10,Qtemp1$STO)			                                          # calculate the Q correlation coefficient r
  Q_var  <-  (sd(Qtemp1$STO)/mean(Qtemp1$STO)) / (sd(Qtemp1$SW_10)/mean(Qtemp1$SW_10))    # variability
  Q_b    <-  mean(Qtemp1$STO)/mean(Qtemp1$SW_10)                                        # bias
  ...
  MAE1 <- mean(sum(abs(Qtemp1$STO - Qtemp1$SW_10)))
  
  Qtemp2  <- na.omit(data.frame(SW_30[366:nrun], GW[366:nrun]))								        # create time frame without missing discharge time steps
  Q2_Rsqu <- cor(Qtemp2$SW_30,Qtemp2$GW)			                                          # calculate the Q correlation coefficient r
  Q2_var  <-  (sd(Qtemp2$GW)/mean(Qtemp2$GW)) / (sd(Qtemp2$SW_30)/mean(Qtemp2$SW_30))    # variability
  Q2_b    <-  mean(Qtemp2$GW)/mean(Qtemp2$SW_30)                                        # bias
  ...
  MAE2 <- mean(sum(abs(Qtemp2$GW - Qtemp2$SW_30)))
  
  Qtemp3  <- na.omit(data.frame(SW_100[366:nrun], Sdeep[366:nrun]))								        # create time frame without missing discharge time steps
  Q3_Rsqu <- cor(Qtemp3$SW_100,Qtemp3$Sdeep)			                                          # calculate the Q correlation coefficient r
  Q3_var  <-  (sd(Qtemp3$Sdeep)/mean(Qtemp3$Sdeep)) / (sd(Qtemp3$SW_100)/mean(Qtemp3$SW_100))    # variability
  Q3_b    <-  mean(Qtemp3$Sdeep)/mean(Qtemp3$SW_100)                                        # bias
  ...
  MAE3 <- mean(sum(abs(Qtemp3$Sdeep - Qtemp3$SW_100)))
  
  Qtemp4  <- na.omit(data.frame(Tobs[366:nrun], Tr[366:nrun]))								        # create time frame without missing discharge time steps
  ...
  Qtemp5  <- na.omit(data.frame(SW_10_D[366:nrun], fupCSTO_D[366:nrun]))								        # create time frame without missing discharge time steps
  Q5_Rsqu <- cor(Qtemp5$SW_10_D,Qtemp5$fupCSTO_D)			                                          # calculate the Q correlation coefficient r
  Q5_var  <-  (sd(Qtemp5$fupCSTO_D)/mean(Qtemp5$fupCSTO_D)) / (sd(Qtemp5$SW_10_D)/mean(Qtemp5$SW_10_D))    # variability
  Q5_b    <-  mean(Qtemp5$fupCSTO_D)/mean(Qtemp5$SW_10_D)                                        # bias
  ...
  MAE5 <- mean(sum(abs(Qtemp5$fupCSTO_D - Qtemp5$SW_10_D)))
  
  Qtemp6  <- na.omit(data.frame(SW_30_D[366:nrun], gwCSTO_D[366:nrun]))								        # create time frame without missing discharge time steps
  Q6_Rsqu <- cor(Qtemp6$SW_30_D,Qtemp6$gwCSTO_D)			                                          # calculate the Q correlation coefficient r
  Q6_var  <-  (sd(Qtemp6$gwCSTO_D)/mean(Qtemp6$gwCSTO_D)) / (sd(Qtemp6$SW_30_D)/mean(Qtemp6$SW_30_D))    # variability
  Q6_b    <-  mean(Qtemp6$gwCSTO_D)/mean(Qtemp6$SW_30_D)                                        # bias
  ...
  MAE6 <- mean(sum(abs(Qtemp6$gwCSTO_D - Qtemp6$SW_30_D)))
  
  Qtemp7  <- na.omit(data.frame(SW_100_D[366:nrun], lowCSTO_D[366:nrun]))								        # create time frame without missing discharge time steps
  Q7_Rsqu <- cor(Qtemp7$SW_100_D,Qtemp7$lowCSTO_D)			                                          # calculate the Q correlation coefficient r
  Q7_var  <-  (sd(Qtemp7$lowCSTO_D)/mean(Qtemp7$lowCSTO_D)) / (sd(Qtemp7$SW_100_D)/mean(Qtemp7$SW_100_D))    # variability
  Q7_b    <-  mean(Qtemp7$lowCSTO_D)/mean(Qtemp7$SW_100_D)                                        # bias
  ...
  MAE7 <- mean(sum(abs(Qtemp7$lowCSTO_D - Qtemp7$SW_100_D)))

  return (cbind(KGE1, Q_Rsqu, MAE1,KGE2, Q2_Rsqu, MAE2, Q3_Rsqu, KGE3, MAE3, 
                Q4_Rsqu, KGE4, MAE4, Q5_Rsqu, KGE5, MAE5, Q6_Rsqu, KGE6, MAE6, Q7_Rsqu, KGE7, MAE7))                  # return evaluation criteria for multi-objective calibration
