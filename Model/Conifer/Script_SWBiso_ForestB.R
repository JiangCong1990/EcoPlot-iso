...
inp <- read.csv("Demnitz_inpmod_FoB.csv",na.strings="", sep = ",")     		    # read input into dataframe from .csv file with , as decimal separator
...
parRange <- data.frame(min=c(-0.6, 0.1, 40, 40,  1,  1,  500,  50,   50, 1.0, 1.0, 1.0, 0.1, 0.5, 1,  3,  10,  0.25, 0.25, 0),
                       max=c(-0.1, 2.0, 60, 60, 20, 20, 1000, 100,  300, 5.0, 5.0, 5.0, 0.9, 1.0, 20, 40, 100, 0.90, 0.75, 2))     # specify parameter ranges from min to max for alpha and beta parameters of the gamma df
...
# === Keep all values within original parameter ranges ===
#LH[, smax_idx]  <- pmax(pmin(LH[, smax_idx],  parRange["Smax",  "max"]), parRange["Smax",  "min"])
#LH[, gwmax_idx] <- pmax(pmin(LH[, gwmax_idx], parRange["GWmax", "max"]), parRange["GWmax", "min"])
#LH[, lmax_idx]  <- pmax(pmin(LH[, lmax_idx],  parRange["Lmax",  "max"]), parRange["Lmax",  "min"])
...
observed<- read.csv("Demnitz_inpmod_FoB.csv", sep = ",")
